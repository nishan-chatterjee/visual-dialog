...
# the embeddings for val aren't generated yet

if check_flag(config['dataset'], 'v0.9'):
    val_dataset.dense_ann_feat_reader = None

val_dataloader = DataLoader(val_dataset, 
                            batch_size=int(config['solver']['batch_size']/2 * torch.cuda.device_count()), 
...
for epoch in range(start_epoch, config['solver']['num_epochs']):
    ...
    for batch in tqdm(train_dataloader, total=iterations, unit="batch"):
        ...
        epoch_loss += (batch["ans"].size(0) * batch_loss.detach()).to(torch.device('cpu'))
